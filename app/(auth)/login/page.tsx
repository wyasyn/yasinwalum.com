...
import { OfflineLoginAccess } from "@/components/local-first/offline-login-access";
import { LoginSessionReset } from "@/components/local-first/login-session-reset";
...
function getErrorMessage(error?: string, email?: string) {
  ...
  if (
    error === "social_sign_in_failed" ||
    error === "social_redirect_missing"
  ) {
...
export default async function LoginPage({ searchParams }: LoginPageProps) {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-6xl items-center p-4 lg:p-8">
      <LoginSessionReset loggedOut={resolvedSearchParams?.logged_out === "1"} />
      <div className="grid w-full overflow-hidden rounded-2xl border bg-card lg:min-h-190 lg:grid-cols-2">
        <section
          className="relative min-h-70 bg-cover bg-center p-6 lg:min-h-full lg:p-8"
          style={{
            backgroundImage:
        ...
        >
          <div className="flex items-center gap-3 text-sm font-medium text-white">
            <span className="grid size-8 place-items-center rounded-full bg-white/95 text-foreground">
              Y
            </span>
          ...
          <div className="absolute bottom-6 left-6 right-6 lg:bottom-8 lg:left-8 lg:right-8">
            <h2 className="text-3xl font-semibold tracking-tight text-white">
              Show your best work professionally
            </h2>
            <p className="mt-3 text-sm text-white/90">
              Manage projects, skills, and markdown posts from one secure
              dashboard.
            </p>
          </div>
        </section>
        <section className="flex w-full flex-col justify-center p-6 lg:p-8">
          <div className="mx-auto w-full max-w-md space-y-6">
            ...
            <form action={loginAction} className="space-y-4">
              ...
              {resolvedSearchParams?.logged_out === "1" ? (
                <p className="text-sm text-emerald-600">
            ...
            <OfflineLoginAccess adminEmail={env.ADMIN_EMAIL} />

            {resolvedSearchParams?.error === "email_not_verified" ||
